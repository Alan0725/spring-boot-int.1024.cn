...
public class CustomerCredentialsMatcher extends SimpleCredentialsMatcher {
    ...
    public boolean doCredentialsMatch(AuthenticationToken authToken, AuthenticationInfo info) {
        ...
        log.debug("{} == {} {}", new String(token.getPassword()).trim(), info.getCredentials().toString(), new BCryptPasswordEncoder().matches(new String(token.getPassword()).trim(), info.getCredentials().toString()));
